...
import reman.client.gui.ui.CustomTabbedPaneUI;
...
public class BasicTabbedPane extends JTabbedPane {
  ...
  public BasicTabbedPane() {
	...
	if (tab_bounds.contains(mouse)) {
	  tabClicked(e);
	} else if (this_.getTabPlacement() == TOP
	    && mouse.y < tab_bounds.getHeight()) {
	  tabAreaClicked(e);
	} else if (this_.getTabPlacement() == BOTTOM
	    && mouse.y > tab_bounds.getY()) {
	  tabAreaClicked(e);
	} else if (this_.getTabPlacement() == LEFT
	    && mouse.x < tab_bounds.getWidth()) {
	  tabAreaClicked(e);
	} else if (this_.getTabPlacement() == RIGHT
	    && mouse.x > tab_bounds.getX()) {
	  tabAreaClicked(e);
	}
      }
    });
  ...
  public void addTab(String title, Icon icon, Component component, String tip,
    ...
    close.setIcon(new ImageIcon(this.getClass().getResource(
	"resources/tab_close_icon.png")));
    close.setRolloverIcon(new ImageIcon(this.getClass().getResource(
	"resources/tab_close_icon_over.png")));
    close.setPressedIcon(new ImageIcon(this.getClass().getResource(
	"resources/tab_close_icon_pressed.png")));
	...
	if (confirm_tab_close_) {
	  int r = JOptionPane.showConfirmDialog(BasicTabbedPane.this,
	      confirm_tab_close_message_, "Tab Closing",
    ...
    if (placement == LEFT) {
      ...
      layout
	  .putConstraint(SpringLayout.EAST, close, -5, SpringLayout.EAST, tab);
    }
    tab.add(close);
    ...
    if (placement == LEFT) {
      layout
	  .putConstraint(SpringLayout.EAST, label, -5, SpringLayout.EAST, tab);
    } else {
  ...
  public void setTabVisible(int i, boolean flag) {
    if (flag) {
      HiddenTab t = hidden_tabs_.remove(hidden_tabs_.indexOf(new HiddenTab(i,
	  null, null, null, null)));
      this.addTab(t.title_, t.icon_, t.panel_, t.tip_, t.index_);
    } else {
      hidden_tabs_.add(new HiddenTab(i, this.getTitleAt(i), this
	  .getComponentAt(i), this.getToolTipTextAt(i), this.getIconAt(i)));
      this.removeTabAt(i);
    }
  }
  ...
  public static void main(String[] args) {
	...
	System.out
	    .println(((BasicTabbedPane) e.getSource()).getSelectedIndex());
      }
    });
