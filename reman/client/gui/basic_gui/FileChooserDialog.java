...
public class FileChooserDialog {
  ...
  public FileChooserDialog() {
    ...
    String path = FrameworkProperties.instance().getProperty(
	"framework.last-directory", System.getProperty("user.home"));
  ...
  public void showDialog(Type type, boolean directories_only) {
    ...
    switch (type) {
    case SAVE:
      selected_ = (chooser_.showSaveDialog(Launcher.instance().getFramework()
	  .getWindow()) == JFileChooser.APPROVE_OPTION);
      break;
    case OPEN:
      selected_ = (chooser_.showOpenDialog(Launcher.instance().getFramework()
	  .getWindow()) == JFileChooser.APPROVE_OPTION);
      break;
    }
    File f = chooser_.getSelectedFile();
    if (f != null) {
      FrameworkProperties.instance().setProperty("framework.last-directory",
	  f.getAbsolutePath());
    }
